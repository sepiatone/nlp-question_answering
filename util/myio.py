...
from tmp import EmbeddingLayer
...
def read_corpus(path):
    ...
    with fopen(path, mode = 'rt', encoding = "utf-8") as fin:
        for line in fin:
            ...
            if len(title) == 0:
                print("empty title for ", idx)
                empty_cnt += 1
                continue
            title = title.strip().split()
            body = body.strip().split()
            raw_corpus[idx] = (title, body)
            # print(idx, title, body)
...
#   cnt = Counter(w for id, pair in raw_corpus.iteritems() for x in pair for w in x)
    cnt = Counter(w for idx, pair in raw_corpus.items() for x in pair for w in x)
    cnt[unk] = cut_off + 1
...
def create_idf_weights(corpus_path, embedding_layer):
    ...
    with fopen(corpus_path, mode = 'rt', encoding = "utf-8") as fin:
        for line in fin:
...
def map_corpus(raw_corpus, embedding_layer, max_len=100):
    ids_corpus = { }
    # for idx, pair in raw_corpus.iteritems():
    for idx, pair in raw_corpus.items():
...
def read_annotations(path, K_neg=20, prune_pos_cnt=10):
    lst = [ ]
    with open(path, mode = "rt", encoding = "utf-8") as fin:
        for line in fin:
            parts = line.split("\t")
            pid, pos, neg = parts[:3]
            pos = pos.split()
...
def create_batches(ids_corpus, data, batch_size, padding_id, perm=None, pad_left=True):
    ...
    for u in range(N):
        idx_1 = perm[u]
        pid, qids, qlabels = data[idx_1]
        if pid not in ids_corpus: continue
        cnt += 1
        for idx_2 in [pid] + qids:
            if idx_2 not in pid2id:
                if idx_2 not in ids_corpus: continue
                pid2id[idx_2] = len(titles)
                t, b = ids_corpus[idx_2]
                titles.append(t)
                bodies.append(b)
        pid = pid2id[pid]
